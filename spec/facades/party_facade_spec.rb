...
RSpec.describe PartyFacade do
  ...
  describe 'player in party?' do
    it 'checks if a player is in a party and returns true if they are' do
      ...
      stub_request(:get, "http://localhost:3000/api/v1/party?user_id=12345").to_return(status: 200, body: json_response)
      stub_request(:get, "http://localhost:3000/api/v1?player_id=1").to_return(status: 200, body: json_response_2)
    ...
    it 'checks if a player is in a party and returns false if they are not' do
      ...
      stub_request(:get, "http://localhost:3000/api/v1/party?user_id=12345").to_return(status: 200, body: json_response)
      stub_request(:get, "http://localhost:3000/api/v1?player_id=4").to_return(status: 200, body: json_response_2)
