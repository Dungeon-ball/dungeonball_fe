...
RSpec.describe 'player show page' do
    ...
    it 'Shows the attributes of a given player' do
    ...
    it 'has a button to add player to party if logged in  ' do
      ...
      stub_request(:get, "http://localhost:3000/api/v1/players/1").to_return(status: 200, body: json_response)
      stub_request(:get, "http://localhost:3000/api/v1/party?query=12345").to_return(status: 200, body: json_response2)

      OmniAuth.config.test_mode = true
      Rails.application.env_config['omniauth.auth'] = OmniAuth.config.mock_auth[:twitter]
      visit '/'
      click_link("Sign in with Twitter")
      #user will have to search for a player's name to navigate to that player's show page.
      visit '/players/1'
    ...
    it 'has a button to add player to party that adds player to a party  ' do
      ...
      json_response3 = File.read('spec/fixtures/empty_party.json')
      json_response4 = File.read('spec/fixtures/party_show_12345.json')
      stub_request(:get, "http://localhost:3000/api/v1/players/1").to_return(status: 200, body: json_response)
      stub_request(:post, "http://localhost:3000/api/v1/parties/12345/players?query=1").to_return(status: 200, body: json_response2)
      stub_request(:get, "http://localhost:3000/api/v1/party?query=12345").to_return(status: 200, body: json_response3)
      stub_request(:get, "http://localhost:3000/api/v1/party?query=12345").to_return(status: 200, body: json_response4)
